...
export const BOOKING_POLICIES: Record<BookingPolicySlug, BookingPolicyMeta> = {
  ...
  "booking-payment-terms": {
    ...
    body: [
      ...
      { kind: "p", text: "Online payments are processed by a regulated payment provider (such as Stripe). Propvora does not store your full card number. Where the host takes payment off-platform, that arrangement is directly between you and the host." },
      { kind: "h2", text: "Failed or disputed payments" },
      { kind: "p", text: "If a payment fails, the booking may not be confirmed or may be cancelled. Do not initiate a chargeback before contacting the host — most issues can be resolved directly, and a record of the booking and accepted policies is available." },
    ],
  },
  "guest-data-notice": {
