  ...
  return (
    <div className="flex flex-col gap-5 px-4 md:px-6 py-5">
      <MobileTopBar
        ...
        primaryAction={{ label: "Create RFQ", icon: Plus, href: "/app/work/orders?tab=quotes" }}
        overflowActions={[{ label: "Escrow management", icon: ShieldCheck, href: "/app/money/escrow" }]}
      />
      ...
      <div className="hidden md:block">
        <SectionHeader
          ...
          actions={
            <>
              <Link href="/app/work/orders?tab=quotes" className="inline-flex items-center gap-1.5 bg-[#2563EB] hover:bg-[#1d4ed8] text-white rounded-xl px-3.5 py-2 text-sm font-semibold transition-colors">
                <Plus className="w-4 h-4" /> Create RFQ
              </Link>
              <Link href="/app/money/escrow" className="inline-flex items-center gap-1.5 border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 rounded-xl px-3.5 py-2 text-sm font-semibold transition-colors">
