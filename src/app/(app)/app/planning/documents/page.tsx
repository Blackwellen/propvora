  ...
  return (
    <div
      ...
      className={cn(
        ...
        dragging ? "border-[#7C3AED] bg-violet-50" : "border-slate-200 bg-slate-50/50 hover:border-violet-300 hover:bg-violet-50/50"
      )}
    >
      <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-3 transition-all", dragging ? "bg-[#7C3AED]/20" : "bg-slate-100 group-hover:bg-violet-100")}>
        <Upload className={cn("w-6 h-6 transition-colors", dragging ? "text-[#7C3AED]" : "text-slate-400 group-hover:text-violet-500")} />
      </div>
      <p className="text-sm font-semibold text-slate-900">{dragging ? "Drop files here" : "Drag & drop files to upload"}</p>
      <p className="text-xs text-slate-500 mt-1">PDF, Word, Excel, ZIP, Images — up to 50MB each</p>
      <button className="mt-3 text-xs font-semibold text-[#7C3AED] hover:underline underline-offset-2">Or click to browse</button>
    </div>
  )
}
...
function FilterBar({ search, onSearch, typeFilter, onTypeFilter }: FilterBarProps) {
  return (
...
export default function DocumentsPage() {
  ...
  // planning_documents is not provisioned in the live DB (42P01 → empty).
  // Query is wrapped so any error yields an honest empty state, never mock fill.
  useEffect(() => {
      ...
      try {
        ...
        setDocs(
          (data as Record<string, unknown>[]).map((d) => ({
            id: String(d.id),
            name: String(d.name ?? d.file_name ?? "Untitled"),
            type: String(d.type ?? d.doc_type ?? "Other"),
            size: String(d.size ?? "—"),
            planningSet: String(d.planning_set_title ?? "—"),
            uploadedAt: d.created_at ? new Date(String(d.created_at)).toLocaleDateString("en-GB") : "—",
  ...
  const filtered = useMemo(
    ...
    [docs, search, typeFilter, archivedHidden]
  )
  ...
  return (
    <DashboardContainer>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 bg-white rounded-2xl border border-slate-200 shadow-sm px-5 py-3.5">
          <div className="flex items-center gap-5 flex-wrap">
            <div>
              ...
              <p className="text-sm font-bold text-slate-900">{loading ? "—" : `${docs.length} files`}</p>
            </div>
          </div>
        ...
        {/* Filter bar */}
        <FilterBar search={search} onSearch={setSearch} typeFilter={typeFilter} onTypeFilter={setTypeFilter} />
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">{loading ? "Loading…" : `${filtered.length} of ${docs.length} shown`}</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center px-4">
              ...
              <p className="text-sm font-semibold text-slate-900">
                {docs.length === 0 ? "No planning documents yet" : "No documents match your filters"}
              </p>
              <p className="text-xs text-slate-400 mt-1 max-w-sm">
                {docs.length === 0
                  ? "Upload contracts, compliance quotes, offer letters and models to keep your planning paperwork in one place."
                  : "Try adjusting your search or filter criteria."}
              </p>
              {docs.length > 0 && (
                <button onClick={() => { setSearch(""); setTypeFilter("All Types") }} className="mt-3 text-xs font-medium text-[#7C3AED] hover:underline">Clear filters</button>
              )}
            </div>
          ) : (
