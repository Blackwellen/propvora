...
function TaskRow({ task }: { task: DisplayTask }) {
  // DB values may fall outside the known enum — never dereference a missing key.
  const priorityCfg = PRIORITY_CONFIG[task.priority] ?? { label: task.priority ?? "—", cls: "bg-slate-100 text-slate-600" }
  const statusCfg = STATUS_CONFIG[task.status] ?? { label: task.status ?? "—", cls: "bg-slate-100 text-slate-600" }
  return (
