...
export default function ComplianceOverviewPage() {
  ...
  return (
    <>
      ...
      {loading ? (
        /* Loading skeleton — mirrors the main content grid to avoid layout shift
           and stop positive empty-state copy from flashing before data arrives. */
        <div className="px-4 sm:px-6 pb-6 grid grid-cols-1 md:grid-cols-12 gap-4" aria-busy="true" aria-label="Loading compliance overview">
          {["md:col-span-4", "md:col-span-4", "md:col-span-4", "md:col-span-7", "md:col-span-5"].map((span, i) => (
            <div
              key={i}
              className={`${span} bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden`}
            >
