  ...
  return (
    <>
      ...
      <div className="hidden md:flex px-6 pb-3 items-center gap-2 flex-wrap">
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-600 bg-white focus:outline-none">
          {STATUS_FILTERS.map((s) => (
            <option key={s} value={s}>{s ? humaniseType(s) : "All statuses"}</option>
          ))}
        </select>
        <select value={riskFilter} onChange={(e) => setRiskFilter(e.target.value)} className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-600 bg-white focus:outline-none">
          {RISK_FILTERS.map((r) => (
