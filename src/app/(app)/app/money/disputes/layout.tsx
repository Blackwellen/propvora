/**
 * Disputes route gate — server component layout
 *
 * The unified dispute lifecycle is a V2 marketplace surface, gated behind
 * `marketplaceDisputes`. When the flag is off, direct URL access to
 * /property-manager/money/disputes redirects to /property-manager/money (the
...
export default async function DisputesLayout({ children }: { children: React.ReactNode }) {
  ...
  const enabled = await isFeatureEnabled("marketplaceDisputes", { supabase })
