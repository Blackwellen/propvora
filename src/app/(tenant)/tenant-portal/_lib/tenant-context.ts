...
export const TENANT_CONTACT_TYPES = ["tenant", "post_tenant", "applicant"] as const
...
export async function resolveTenantContext(): Promise<TenantContext | null> {
  ...
  try {
    const { data: access, error } = await supabase
      ...
      .eq("user_id", user.id)
      .in("access_type", TENANT_ACCESS_TYPES as unknown as string[])
      .eq("status", "active")
      .limit(1)
      .maybeSingle()
    ...
    if (access?.contact_id) {
      ...
      return {
        ...
        displayName: c?.displayName ?? "Tenant",
  ...
  // 2. Email-match fallback against live tenant-type contacts
  if (user.email) {
    try {
...
async function loadContact(
  ...
  try {
    ...
    return {
      ...
      displayName: (data?.display_name as string) ?? "Tenant",
