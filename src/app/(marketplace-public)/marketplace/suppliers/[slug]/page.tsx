...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!l) return { title: "Supplier · Propvora Marketplace" }
  return {
    title: `${l.title} · Propvora Marketplace`,
    description: l.description?.slice(0, 160) ?? "Request a quote from this vetted supplier.",
...
function NotFound() {
  return (
    <div className="mx-auto max-w-xl px-6 py-20 text-center">
      ...
      <h1 className="text-[19px] font-bold text-[#0B1B3F]">This supplier isn&apos;t available</h1>
      <p className="mt-2 text-[13.5px] text-slate-500">It may have been unpublished or is no longer taking enquiries.</p>
