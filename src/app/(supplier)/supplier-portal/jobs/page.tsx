...
import {
  ...
  jobStatusMeta, buildJobLabelMaps,
...
function priorityBadge(priority: string) {
  if (priority === "high" || priority === "urgent") return <Badge variant="danger" size="sm">High Priority</Badge>
    ...
    async function load() {
      try {
        ...
        const { data, error: fetchErr } = await supabase
          .from("jobs")
          .select("id, title, description, status, priority, scheduled_date, reference, category, property_id, contact_id, quoted_amount, approved_amount")
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">{jobs.length} total job{jobs.length === 1 ? "" : "s"} assigned to you</p>
        </div>
      </div>
                      ...
                      <div className="flex items-center gap-4 mt-2.5 flex-wrap">
                        {job.propertyLabel && (
                          <span className="flex items-center gap-1.5 text-xs text-slate-500">
                            <Building2 className="w-3.5 h-3.5 shrink-0" />{job.propertyLabel}
                          </span>
                        )}
                        {job.scheduled_date && (
                          <span className="flex items-center gap-1.5 text-xs text-slate-500">
                            <Calendar className="w-3.5 h-3.5 shrink-0" />{formatDate(job.scheduled_date)}
                          </span>
                        )}
                        {job.operatorLabel && (
                          <span className="flex items-center gap-1.5 text-xs text-slate-500">
                            <User className="w-3.5 h-3.5 shrink-0" />{job.operatorLabel}
                          </span>
                        )}
                        {amount != null && (
                          <span className="flex items-center gap-1.5 text-xs font-medium text-slate-600">
                            <PoundSterling className="w-3.5 h-3.5 shrink-0" />{formatMoney(amount)}
                          </span>
                        )}
                      </div>
                      {job.category && (
      ...
      {filtered.length > 0 && (
        <div className="pt-2">
          <span className="text-xs text-slate-500">Showing {filtered.length} of {jobs.length} jobs</span>
        </div>
      )}
    </div>
  )
