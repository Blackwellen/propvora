...
export async function generateMetadata({
  ...
  return {
    title: `${rental.title} · My Rentals · Propvora`,
    description:
      rental.description ??
      `${rental.beds}-bed ${rental.propertyType} in ${rental.location}`,
  }
}

export default async function CustomerLongTermRentalDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-5">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <button className="flex items-center gap-1.5 text-sm font-medium border border-slate-200 rounded-lg px-3 py-1.5 hover:bg-slate-50 text-slate-600 transition-colors">
            ...
            <button className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 px-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {rental.nearbyAmenities.map((a) => (
                  <span key={a} className="text-sm bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full">
                    {a}
                  </span>
                ))}
              </div>
        ...
        {/* Right: enquiry panel with isAuthenticated=true */}
        <div>
          <LongTermRentalEnquiryPanel
            rental={rental}
