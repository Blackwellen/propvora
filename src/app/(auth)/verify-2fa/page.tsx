...
function Verify2FAInner() {
  ...
  const handleResend = async () => {
    if (resendCooldown > 0) return
    // TOTP codes are generated locally in the authenticator app — there is nothing
    // to "resend". This button triggers a helpful tip to open the app and wait for
    // the next 30-second code cycle, then starts a brief cooldown so it can't spam.
    ...
    <>
      ...
      <div className="mt-5 text-center">
        <p className="text-sm text-slate-500">
          ...
          >
            {resendCooldown > 0 ? `Try again in ${resendCooldown}s` : "Get a new code"}
          </button>
        </p>
        {resendCooldown > 0 ? (
          <p className="mt-1 text-xs text-slate-400">
            Open your authenticator app — a new 6-digit code refreshes every 30 seconds.
          </p>
        ) : (
          <p className="mt-1 text-xs text-slate-400">
            Codes are generated by your authenticator app — no email needed.
          </p>
        )}
      </div>
