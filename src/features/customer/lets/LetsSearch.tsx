...
const RESULTS = [
  ...recommendedLets,
  { id: "victoria-riverside", title: "Victoria Riverside", location: "Manchester, M4", image: IMG.cityLoft, rentPence: 170000, beds: 2, baths: 2, furnished: true, available: "Available now" },
  { id: "hilltop-lofts", title: "Hilltop Lofts", location: "Ancoats, M4", image: IMG.urban, rentPence: 138000, beds: 1, baths: 1, furnished: true, available: "Available 1 Jul" },
]
const KPIS = [
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-[15px] font-bold text-slate-900">Long-term lets <span className="text-slate-400 font-medium text-[13px]">120 results</span></h3>
      ...
      <div className={cn("grid gap-5 items-start", view === "map" ? "grid-cols-1 lg:grid-cols-[1fr_420px]" : "")}>
        <div className={cn("grid gap-4", view === "list" ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3", view === "map" && "lg:grid-cols-2")}>
          {RESULTS.map((l) => (
            <CustomerPropertyCard key={l.id} p={{ id: l.id, title: l.title, location: l.location, image: l.image, pricePence: l.rentPence, pricePer: "month", badge: l.available, href: `/customer/lets/properties/${l.id}` }} onToggleSave={() => toast("Saved to favourites", "success")} />
          ))}
        </div>
        {view === "map" && (
