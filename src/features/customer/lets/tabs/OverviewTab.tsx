...
const KPIS = [
  { id: "viewings", label: "Upcoming viewings", value: "—", sub: "No viewings yet", icon: Calendar, bg: "bg-[var(--brand-soft)] text-[var(--brand)]", href: "/customer/lets?tab=viewings" },
...
const TONE: Record<string, string> = { emerald: "bg-emerald-50 text-emerald-600", blue: "bg-[var(--brand-soft)] text-[var(--brand)]", amber: "bg-amber-50 text-amber-600", violet: "bg-violet-50 text-violet-600" }
            ...
            <ol className="flex items-start justify-between gap-2">
              {JOURNEY.map((s, i) => { const Icon = s.icon; return (
                <li key={s.n} className="flex-1 flex flex-col items-center text-center relative">
                  {i < JOURNEY.length - 1 && <span className={cn("absolute top-5 left-1/2 w-full h-0.5", s.state === "done" ? "bg-[var(--color-brand-400)]" : "bg-white/15")} />}
                  <span className={cn("w-10 h-10 rounded-full flex items-center justify-center z-10 shrink-0", s.state === "done" ? "bg-[var(--brand)]" : s.state === "current" ? "bg-white text-[#0D1B2A]" : "bg-white/10 text-white/50")}><Icon className="w-4 h-4" /></span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        {KPIS.map((k) => { const Icon = k.icon; return (
          <div key={k.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            ...
            <Link href={k.href} className="text-[11px] font-semibold text-[var(--brand)] mt-1 inline-block">View all →</Link>
          </div>
        )})}
      </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div className="flex items-center justify-between mb-1">
              ...
              <Link href="/customer/lets" className="text-[12px] font-semibold text-[var(--brand)]">Browse all lets →</Link>
            </div>
            <p className="text-[12.5px] text-slate-500 mb-3">Curated long-term lets based on your preferences.</p>
            {rentals.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
                <span className="w-12 h-12 rounded-2xl bg-[var(--brand-soft)] text-[var(--brand)] flex items-center justify-center"><Home className="w-6 h-6" /></span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-100">
              {TRUST.map((t) => { const Icon = t.icon; return <div key={t.title} className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-[var(--brand-soft)] text-[var(--brand)] flex items-center justify-center shrink-0"><Icon className="w-4 h-4" /></span><div><p className="text-[11.5px] font-semibold text-slate-800">{t.title}</p><p className="text-[10.5px] text-slate-400">{t.sub}</p></div></div> })}
            </div>
          </div>
        ...
        <aside className="space-y-5">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            ...
            <div className="flex flex-col items-center justify-center py-6 text-center gap-2">
              ...
              <Link href="/customer/account-settings" className="text-[12px] font-semibold text-[var(--brand)] hover:underline">Complete profile →</Link>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3"><h3 className="text-[14px] font-bold text-slate-900">Account summary</h3><Link href="/customer/account-settings" className="text-[12px] font-semibold text-[var(--brand)]">View profile →</Link></div>
            <p className="text-[12.5px] text-slate-400">Complete your profile to personalise your letting experience.</p>
            <Link href="/customer/account-settings" className="mt-3 inline-block text-[12px] font-semibold text-[var(--brand)] hover:underline">Set up profile →</Link>
          </div>
