...
import {
  Key, Truck, Wallet, FolderOpen, Search, ChevronRight, FileText, MessageSquare,
  RotateCcw, LogOut, Wrench, CheckCircle2, Loader2,
} from "lucide-react"
import { cn } from "@/lib/utils"
import { formatPence } from "@/lib/marketplace/money"
import { StatusPill, type PillTone } from "../../components/StatusPill"
import type { Tenancy, TenancyStatus } from "../../data/lets"
...
export default function TenancyTab() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/customer/lets?type=tenancies", { headers: { accept: "application/json" } })
        const j = (await res.json()) as { tenancies?: Tenancy[] }
        const rows = j.tenancies ?? []
        if (!active) return
        setData(rows)
  ...
  const selected = data.find((t) => t.id === selectedId) ?? data[0] ?? null
  const rentDue = data.filter((t) => t.status === "Active").reduce((s, t) => s + t.rentPence, 0)
        ...
        <aside className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sticky top-[84px]">
          ...
          {selected.nextPaymentDate !== "—" && (
            <div className="mt-3 pt-3 border-t border-slate-100">
              <p className="text-[12px] font-semibold text-slate-700 mb-1.5">Upcoming</p>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100">
            ...
            <ol className="space-y-1.5">
              ...
              {selected.nextPaymentDate !== "—" && <TL title="Next rent payment" sub={selected.nextPaymentDate} />}
            </ol>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 gap-2">
            ...
            <QA icon={FileText} label="Documents" href={`/customer/lets/tenancies/${selected.id}/documents`} />
            <QA icon={Wrench} label="Report maintenance" href={`/customer/lets/tenancies/${selected.id}/maintenance`} />
            <QA icon={MessageSquare} label="Message manager" href="/customer/messages" />
            <QA icon={Wallet} label="Pay / rent history" href={`/customer/lets/tenancies/${selected.id}/rent-payments`} />
            <QA icon={RotateCcw} label="Renew tenancy" href="/customer/messages" />
            <QA icon={LogOut} label="Give notice" href="/customer/messages" />
          </div>
        </aside>
        )}
      </div>
...
function QA({ icon: Icon, label, href }: { icon: typeof FileText; label: string; href: string }) {
  return <Link href={href} className="inline-flex items-center justify-center gap-1.5 border border-slate-200 rounded-xl py-2 text-[11px] font-semibold text-slate-700 hover:bg-slate-50"><Icon className="w-3.5 h-3.5" /> {label}</Link>
}
