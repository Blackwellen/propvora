  ...
  return (
    <div className="space-y-5 pb-16">
      ...
      <div className="flex items-center gap-1 border-b border-slate-200 overflow-x-auto">
        {TABS.map((t) => <button key={t.id} onClick={() => changeTab(t.id)} className={cn("px-3.5 py-2.5 text-[13.5px] font-semibold border-b-2 -mb-px whitespace-nowrap", t.id === tab ? "border-[var(--brand)] text-[var(--brand)]" : "border-transparent text-slate-500 hover:text-slate-800")}>{t.label}</button>)}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <Stat icon={BadgeCheck} bg="bg-[var(--brand-soft)] text-[var(--brand)]" label="Profile completeness" value="—" cta="Complete profile" onClick={() => changeTab("profile")} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5 items-start">
        <div className="space-y-5">
          ...
          {(tab === "overview" || tab === "finance") && (
            <Panel title="Finance &amp; payment settings" action={<Link href="/customer/payments" className="text-[12px] font-semibold text-[var(--brand)]">Open payments →</Link>}>
        ...
        <aside className="space-y-5">
          ...
          <Panel title="Payment summary">
            ...
            <Link href="/customer/payments" className="mt-1 inline-block text-[12px] font-semibold text-[var(--brand)]">Manage payment methods →</Link>
          </Panel>
        ...
        <div className="fixed bottom-0 inset-x-0 z-40 bg-white border-t border-slate-200 px-4 py-3 md:pl-[calc(50%-740px+2rem)]">
          <div className="mx-auto max-w-[1480px] flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={saveProfile} disabled={savingProfile} className="bg-[var(--brand)] text-white rounded-xl px-4 py-2 text-[12.5px] font-semibold disabled:opacity-60">{savingProfile ? "Saving…" : "Save changes"}</button>
            </div>
          </div>
        </div>
      )}
...
function Stat({ icon: Icon, bg, label, value, cta, onClick, pill }: { icon: typeof UserCheck; bg: string; label: string; value: string; cta: string; onClick: () => void; pill?: boolean }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
      ...
      <button onClick={onClick} className="mt-1.5 text-[11.5px] font-semibold text-[var(--brand)]">{cta}</button>
    </div>
  )
}
