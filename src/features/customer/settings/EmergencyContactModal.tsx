      ...
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center"><LifeBuoy className="w-4 h-4" /></span>
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button onClick={submit} disabled={busy} className="bg-[#2563EB] text-white rounded-xl px-4 py-2 text-[12.5px] font-semibold inline-flex items-center gap-1.5 disabled:opacity-60">{busy && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Save contact</button>
          </div>
        </div>
      </div>
    </div>
