    ...
    () =>
      filtered
        ...
        .map((j) => ({
          ...
          tone: STATUS_DOT[j.status] ?? "bg-[var(--brand)]",
                ...
                {filtered.map((j) => (
                  <tr key={j.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <Td className="text-right">
                      <Link
                        href={`/supplier/jobs/${j.id}`}
                        className="inline-flex items-center gap-0.5 text-xs font-semibold text-[var(--brand)] hover:text-[var(--brand-strong)]"
                      >
