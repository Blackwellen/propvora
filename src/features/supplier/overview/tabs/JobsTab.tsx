        ...
        <aside className="space-y-5">
          ...
          <Panel title="Quick actions">
            <div className="grid grid-cols-2 gap-2.5">
              ...
              <Link href="/supplier/messages" className="flex flex-col items-center gap-1.5 p-3 rounded-xl border border-slate-200 hover:bg-slate-50 text-center"><span className="w-9 h-9 rounded-xl bg-[var(--brand-soft)] flex items-center justify-center"><MessageSquare className="w-4 h-4 text-[var(--brand)]" /></span><span className="text-[11px] font-medium text-slate-700">Messages</span></Link>
              <Link href="/supplier/schedule" className="flex flex-col items-center gap-1.5 p-3 rounded-xl border border-slate-200 hover:bg-slate-50 text-center"><span className="w-9 h-9 rounded-xl bg-[var(--brand-soft)] flex items-center justify-center"><Clock className="w-4 h-4 text-[var(--brand)]" /></span><span className="text-[11px] font-medium text-slate-700">Schedule</span></Link>
            </div>
          </Panel>
...
function JobCard({ j, onSelect, selected }: { j: JobRow; onSelect: (j: JobRow) => void; selected: boolean }) {
  ...
  return (
    <button type="button" onClick={() => onSelect(j)} className={`text-left bg-white border rounded-[18px] shadow-sm p-4 transition-all hover:shadow-md ${selected ? "border-[var(--color-brand-400)] ring-2 ring-[var(--color-brand-100)]" : "border-slate-200 hover:border-slate-300"}`}>
    ...
    <Panel pad={false}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((j) => (
              <tr key={j.id} onClick={() => onSelect(j)} className={`border-b border-slate-50 cursor-pointer hover:bg-slate-50 ${selectedId === j.id ? "bg-[var(--brand-soft)]/50" : ""}`}>
      ...
      <ol className="relative pl-5">
        ...
        {sorted.map((j) => (
          <li key={j.id} className="relative pb-4 last:pb-0">
            <span className={`absolute -left-5 top-1.5 w-3.5 h-3.5 rounded-full border-2 border-white ${j.status === "at_risk" ? "bg-red-500" : "bg-[var(--brand)]"}`} />
            <button onClick={() => onSelect(j)} className={`w-full text-left rounded-xl -mx-2 px-2 py-1.5 hover:bg-slate-50 ${selectedId === j.id ? "bg-[var(--brand-soft)]/50" : ""}`}>
...
function MapView({ rows, onSelect, selectedId }: { rows: JobRow[]; onSelect: (j: JobRow) => void; selectedId?: string }) {
  return (
    <Panel pad={false} className="overflow-hidden">
      ...
      <ul className="divide-y divide-slate-100">
        {rows.map((j) => (
          <li key={j.id}><button onClick={() => onSelect(j)} className={`w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-slate-50 ${selectedId === j.id ? "bg-[var(--brand-soft)]/50" : ""}`}>
            <span className="w-6 h-6 rounded-full bg-[var(--color-brand-100)] text-[var(--brand)] text-[10px] font-bold flex items-center justify-center shrink-0">{j.distanceMiles}</span>
        ...
        return (
          <div key={c.key} className="bg-slate-50 border border-slate-200 rounded-[18px] p-2.5">
            ...
            <div className="space-y-2">
              {items.map((j) => (
                <button key={j.id} onClick={() => onSelect(j)} className={`w-full text-left bg-white border rounded-xl p-2.5 hover:shadow-sm ${selectedId === j.id ? "border-[var(--color-brand-400)]" : "border-slate-200"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-5">
        <div className="space-y-4">
          ...
          <div>
            ...
            <a href={`tel:${j.contactPhone}`} className="text-[12px] font-medium text-[var(--brand)] inline-flex items-center gap-1"><Phone className="w-3 h-3" /> {j.contactPhone}</a>
          </div>
        </div>
        <div className="space-y-4">
          <div>
          ...
          <div className="flex flex-col gap-2">
            <button onClick={() => toast("Job started (demo)", "success")} className="inline-flex items-center justify-center gap-1.5 bg-[var(--brand)] text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-[var(--brand-strong)]"><Play className="w-4 h-4" /> Start job</button>
