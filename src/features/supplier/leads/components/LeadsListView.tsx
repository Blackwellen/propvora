...
import { LeadsSourceFilter, type LeadSourceFilter } from "./LeadsSourceFilter"

// ─── Status tab types ────────────────────────────────────────────────────────

export type StatusTab = "new" | "quoted" | "active" | "completed" | "declined" | "all"
...
export function statusTab(l: SupplierLead): StatusTab {
...
interface LeadsListViewProps {
  /** Pre-filter to a specific status tab. When provided, the status tab rail is hidden. */
  initialStatus?: StatusTab
  /** Whether to show the status tab rail (default true). */
  showStatusTabs?: boolean
}

// ─── Main component ──────────────────────────────────────────────────────────

export function LeadsListView({ initialStatus = "new", showStatusTabs = true }: LeadsListViewProps) {
  ...
  const tabCounts: Record<StatusTab, number> = useMemo(() => {
    const counts: Record<StatusTab, number> = {
      new: 0, quoted: 0, active: 0, completed: 0, declined: 0, all: 0,
    }
  ...
  async function submitQuote() {
    ...
    if (validLines.length === 0) {
      setBanner({ tone: "red", msg: "Add at least one line item." })
      return
    }
    setBusy(true)
    try {
  ...
  async function submitDecline() {
    ...
    try {
      ...
      if (!res.ok) {
        setBanner({ tone: "red", msg: "Couldn't decline this lead." })
        return
      }
    ...
    <div className="space-y-5">
      ...
      {showStatusTabs && (
        <div className="space-y-2">
          ...
          />
          <LeadsSourceFilter value={sourceFilter} onChange={setSourceFilter} />
        </div>
      )}
                  ...
                  <tr key={lead.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <Td className="text-slate-600">
                      {lead.amountPence != null
                        ? moneyPence(lead.amountPence, lead.currency ?? "GBP")
                        : "—"}
                    </Td>
      ...
      <SupplierDrawer
        ...
        footer={
          <div className="flex items-center gap-2">
            <SupplierButton variant="secondary" onClick={() => setViewingLead(null)}>
              Close
            </SupplierButton>
            {viewingLead?.quoteId &&
              (viewingLead.status ?? "").toLowerCase() === "requested" && (
                <SupplierButton
                  onClick={() => {
                    const l = viewingLead
                    setViewingLead(null)
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                <SupplierStatusBadge
                  tone={viewingLead.source === "quote_request" ? "violet" : "blue"}
                >
                  {viewingLead.source === "quote_request"
                    ? "Quote request"
                    : "Marketplace enquiry"}
                </SupplierStatusBadge>
                <SupplierStatusBadge tone={toneForStatus(viewingLead.status)}>
                  {humaniseStatus(viewingLead.status)}
                </SupplierStatusBadge>
              </div>
            </div>
            {viewingLead.detail && (
            ...
            <div className="grid grid-cols-2 gap-3">
              {viewingLead.amountPence != null && (
                <InfoChip
                  icon={Banknote}
                  label="Budget"
                  value={moneyPence(viewingLead.amountPence, viewingLead.currency ?? "GBP")}
                />
              )}
      ...
      <SupplierDrawer
        ...
        footer={
          <>
            ...
            <SupplierButton onClick={submitQuote} loading={busy}>
              Submit quote · {moneyPence(quoteTotal)}
            </SupplierButton>
          </>
        }
      >
          ...
          <div className="space-y-4">
            <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
              ...
              {quoting.amountPence != null && (
                <p className="text-xs text-slate-500 mt-0.5">
                  Buyer budget: {moneyPence(quoting.amountPence, quoting.currency ?? "GBP")}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {quoteLines.map((line, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-[1fr_60px_90px_24px] gap-1.5 items-start"
                  >
                    <input
            ...
            <SupplierField label="Notes (optional)" hint="Scope, assumptions, lead time, exclusions.">
              <textarea
                ...
                placeholder="e.g. Price based on single visit; materials included; 2–3 day lead time."
              />
            </SupplierField>
          </div>
        )}
      ...
      <SupplierDrawer
        ...
        footer={
          <>
            ...
            <SupplierButton onClick={submitDecline} loading={busy}>
              Confirm decline
            </SupplierButton>
          </>
        }
        ...
        {decliningLead && (
          <div className="space-y-4">
            ...
            <SupplierField
              ...
              hint="Helps property managers understand your capacity."
            >
              <textarea
                className={supplierTextareaClass}
                value={declineReason}
  ...
  return (
    <SupplierCard className="p-4 flex flex-col">
      ...
      <div className="mt-2 space-y-1">
        {lead.amountPence != null && (
          <p className="text-xs text-slate-600 flex items-center gap-1">
            <Banknote className="w-3 h-3 text-slate-400" /> Budget:{" "}
            {moneyPence(lead.amountPence, lead.currency ?? "GBP")}
          </p>
        )}
        {lead.counterpartyName && (
      ...
      <div className="mt-3 pt-3 border-t border-slate-100 flex items-center gap-2">
        ...
        {canQuote && (
          <>
            <SupplierButton size="sm" onClick={() => onQuote(lead)}>
              <Send className="w-3.5 h-3.5" /> Quote
            </SupplierButton>
            <SupplierButton size="sm" variant="ghost" onClick={() => onDecline(lead)}>
...
function Th({ children, className }: { children?: React.ReactNode; className?: string }) {
  return (
    <th
      className={`px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-slate-400 ${className ?? ""}`}
    >
      {children}
    </th>
  )
