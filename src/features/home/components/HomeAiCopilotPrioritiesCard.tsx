...
interface HomeAiCopilotPrioritiesCardProps {
  priorities: HomeAiPriority[]
}

export function HomeAiCopilotPrioritiesCard({ priorities }: HomeAiCopilotPrioritiesCardProps) {
  return (
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-white/20 flex items-center justify-center">
            <Sparkles className="text-white" style={{ width: 14, height: 14 }} />
          </div>
          <h3 className="text-[13px] font-semibold text-white">Smart priorities</h3>
        </div>
        <span className="text-[11px] text-violet-200">Derived from your live data</span>
      </div>
