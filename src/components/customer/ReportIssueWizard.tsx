...
export default function ReportIssueWizard({
  ...
  const canContinue =
    step === 1 ? !!category : step === 2 ? subject.trim().length > 2 && detail.trim().length > 4 : step === 3 ? !!severity : true
  ...
  async function submit() {
    ...
    try {
      const res = await fetch("/api/customer/issues", {
        ...
        body: JSON.stringify({ bookingId, category, severity, subject, detail }),
      })
      if (!res.ok) {
        const j = await res.json().catch(() => ({}))
        throw new Error(j.error || "Could not report issue")
      }
      setDone(true)
      router.refresh()
    ...
    >
      ...
      {step === 2 && (
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">A short title and a clear description help us fix it faster.</p>
          <div className="mt-5 space-y-4">
            ...
            <div>
              ...
              <textarea id="detail" value={detail} onChange={(e) => setDetail(e.target.value)} rows={5} placeholder="Describe the problem, when it started and anything you've tried." className={customerTextareaClass} />
            </div>
          </div>
        </div>
      )}
