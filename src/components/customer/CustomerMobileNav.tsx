...
/* ──────────────────────────────────────────────────────────────────────────
   CustomerMobileBottomNav — dedicated bottom tab bar for the customer
   workspace, rendered only below `lg`. It is a DISTINCT bar from the operator
   app nav and the supplier-workspace bar:

   • Slots: Dashboard · Bookings · Orders · More.
   • No raised Copilot centre button (the customer workspace has no global AI
     panel) — the three primary destinations are flat tabs, and Saved /
     Messages / Profile live in the "More" bottom sheet (mirroring the desktop
     side-nav so nothing is lost).
   • Safe-area-inset padding, ≥44px targets, aria-current on the active tab.
─────────────────────────────────────────────────────────────────────────── */

function BottomTab({ dest, active }: { dest: CustomerNavItem; active: boolean }) {
...
export default function CustomerMobileBottomNav() {
  ...
  const moreActive = CUSTOMER_MORE.some((i) => isCustomerNavActive(pathname, i.href))
    ...
    <>
      ...
      >
        <ul className="grid grid-cols-4 items-end">
          {CUSTOMER_PRIMARY.map((dest) => (
            <li key={dest.href}>
              <BottomTab dest={dest} active={isCustomerNavActive(pathname, dest.href)} />
            </li>
          ))}
          <li>
            <button
