...
4. Fixed every concrete defect found (FIX-596 … FIX-602).
...
| **FIX-602** | P1 | Recipes | `useRecipe(featured[2])` crashed when < 3 featured recipes; hardcoded "arrears ~9h/month" claim | Null-safe `featured[2] ?? featured[0] ?? allRecipes[0]`, card hidden when none; honest copy. `RecipesPage.tsx` |
